...
final class Trend implements JsonSerializable
{
    ...
    {
        // If no previous value, return 100% if current is positive (representing new value)
        if (! $this->previous) {
            return bccomp($this->current, '0.0000', 4) > 0 ? '100.00' : '0.00';
        }

        // If previous is zero but current is not, return 100%
        ...
        $division = bcdiv($difference, $this->previous, 6); // Use 6 decimal places for division
