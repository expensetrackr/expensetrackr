...
export function Text({ className, ...props }: React.ComponentPropsWithoutRef<"p">) {
    return <p data-slot="text" {...props} className={cx("text-paragraph-sm text-(--text-sub-600)", className)} />;
}
...
export function Strong({ className, ...props }: React.ComponentPropsWithoutRef<"strong">) {
    return <strong {...props} className={cx(className, "text-zinc-950 font-medium dark:text-white")} />;
}
