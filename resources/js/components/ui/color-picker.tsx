...
function ColorArea({ className, ...props }: AriaColorAreaProps) {
    return (
        <AriaColorArea
            className={composeRenderProps(className, (className) => cn("h-[232px] w-full rounded-8", className))}
            {...props}
        />
    );
}
...
function EyeDropperButton(props: React.ComponentPropsWithRef<"button">) {
