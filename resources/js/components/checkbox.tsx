...
export function Checkbox({
	...
	return (
		<Headless.Checkbox data-slot="control" {...props} className={cx("group inline-flex focus:outline-none", className)}>
			<span
			...
			>
				{props.indeterminate ? (
					<SubtractLineIcon className="size-4 stroke-[var(--checkbox-check)] opacity-0 group-data-[indeterminate]:opacity-100 sm:size-2.5" />
				) : (
					<CheckIcon className="size-4 stroke-[var(--checkbox-check)] opacity-0 group-data-[checked]:opacity-100 sm:size-2.5" />
				)}
			</span>
		</Headless.Checkbox>
	);
